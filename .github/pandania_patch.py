...
new_patch = r'''/* ===== PANDANIA GAMEPLAY PATCH v6 ===== */
(function(){
  ...
  /* Mobile controls: a compact touch D-pad plus attack/interact/bag buttons. */
  function mobileUI(){
    ...
    function hold(btn,key){
      const start=e=>{e.preventDefault();btn.setPointerCapture?.(e.pointerId);keys[key]=true;face(keyDir(key));startMusic?.()};
      const stop=e=>{e.preventDefault();keys[key]=false};
      btn.addEventListener('pointerdown',start);btn.addEventListener('pointerup',stop);btn.addEventListener('pointercancel',stop);btn.addEventListener('pointerleave',e=>{if(e.buttons===0)stop(e)});
    }
    ...
    document.getElementById('pmBag').addEventListener('pointerdown',e=>{e.preventDefault();document.getElementById('bagWindow')?.classList.toggle('open');const b=document.getElementById('bagWindow');if(b)b.style.display=b.style.display==='block'?'none':'block';renderBag?.()});
  }
  mobileUI();

  const oldUpdate=updatePlayer;
  updatePlayer=function(){
    const ox=player.x,oy=player.y;oldUpdate();
    ...
    for(const m of monsters){if(m.dead)continue;const r=(m.collisionRadius||28)+18;if(Math.hypot(player.x-m.x,player.y-m.y)<r){player.x=ox;player.y=oy;break}}
  };

  /* Preserve the existing paper-doll look, recreating it only if absent. */
  function ensureDoll(){
    const bag=document.getElementById('bagWindow'),items=document.getElementById('bagItems');if(!bag||!items)return;
    let p=document.getElementById('pandaniaEquipment');if(p)return p;
    p=document.createElement('div');p.id='pandaniaEquipment';p.style.cssText='margin:0 0 16px;padding:14px;border:2px solid #8b683e;border-radius:12px;background:#15100c;color:#fff';
    ...
    items.parentNode.insertBefore(p,items);return p;
  }
  function setInv(n,c){if(!inventory[n]&&c>0)inventory[n]={count:0};if(inventory[n])inventory[n].count=Math.max(0,c);if(inventory[n]?.count===0)delete inventory[n]}
  function refreshDoll(){
    const p=ensureDoll();if(!p)return;const vals=[['equipWeapon','⚔️','Weapon',state.weapon,'weapon'],['equipArmor','🛡️','Armor',state.armor,'armor'],['equipTool','🛠️','Tool',state.tool,'tool']];
    vals.forEach(([id,ic,label,item,kind])=>{const el=document.getElementById(id);if(!el)return;el.innerHTML=ic+' '+label+'<br><strong>'+(item||'Empty')+'</strong>'+(item?'<br><button class="pdUnequip" type="button">Unequip</button>':'');const b=el.querySelector('.pdUnequip');if(b)b.onclick=e=>{e.stopPropagation();setInv(item,(inventory[item]?.count||0)+1);if(kind==='weapon')state.weapon='';if(kind==='armor')state.armor=null;if(kind==='tool')state.tool=null;refreshDoll();renderBag()}});
    document.querySelectorAll('#bagWindow .itemCard').forEach(card=>{const el=card.querySelector('.itemName');if(!el)return;const n=el.textContent.trim();if(!/^(Wooden Sword|Weapon 1|Weapon 2|Weapon 3|Iron Armor|Woodcutting Axe|Fishing Rod)$/.test(n))return;card.ondblclick=e=>{e.preventDefault();e.stopPropagation();if(!inventory[n])return;if(/^(Wooden Sword|Weapon 1|Weapon 2|Weapon 3)$/.test(n)){if(state.weapon)setInv(state.weapon,(inventory[state.weapon]?.count||0)+1);state.weapon=n}else if(n==='Iron Armor'){if(state.armor)setInv(state.armor,(inventory[state.armor]?.count||0)+1);state.armor=n}else{if(state.tool)setInv(state.tool,(inventory[state.tool]?.count||0)+1);state.tool=n}setInv(n,(inventory[n]?.count||1)-1);refreshDoll();renderBag();showMessage('✅ '+n+' equipped!')}})
  }
  const oldBag=renderBag;renderBag=function(){oldBag();ensureDoll();refreshDoll()};setTimeout(()=>{ensureDoll();refreshDoll()},200);

  /* Robust PDA image: inject an actual <img> into common PDA/token elements
     and use it as the item art. */
  function pdaArt(){
    const urls=['images/pda.png','images/pda.webp','images/pda.jpg'];const probe=new Image();let i=0;
    probe.onload=()=>{
      window.PDA_ART_URL=probe.src;
      document.querySelectorAll('[data-pda],.pda,.pdaIcon,.tokenIcon').forEach(el=>{el.style.backgroundImage='url("'+probe.src+'")';el.style.backgroundSize='contain';el.style.backgroundRepeat='no-repeat';el.style.backgroundPosition='center';el.style.color='transparent';const old=el.querySelector('img[data-pda-art]');if(!old){const im=document.createElement('img');im.src=probe.src;im.dataset.pdaArt='true';im.style.cssText='width:100%;height:100%;max-width:64px;max-height:64px;object-fit:contain;display:block;margin:auto';el.appendChild(im)}});
    };probe.onerror=()=>{i++;if(i<urls.length)probe.src=urls[i]};probe.src=urls[0];
  }pdaArt();

  /* Keep the subtle ambience from v5. */
  ...
  window.__pandaniaGameplayV6=true;
})();
/* ===== END PANDANIA GAMEPLAY PATCH v6 ===== */'''
