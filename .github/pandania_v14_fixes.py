...
@media (max-width:700px),(pointer:coarse){
  ...
  #paperDoll .equippedWeapon,#paperDoll .weaponSlot,#equippedWeapon,#equippedWeaponSlot{position:relative!important;z-index:1000!important;overflow:visible!important;min-width:78px!important;min-height:78px!important;display:flex!important;align-items:center!important;justify-content:center!important;flex:none!important}
  #paperDoll .equippedWeapon img,#paperDoll .weaponSlot img,#equippedWeapon img,#equippedWeaponSlot img{position:relative!important;z-index:1100!important;display:block!important;visibility:visible!important;opacity:1!important;width:76px!important;height:76px!important;max-width:none!important;max-height:none!important;object-fit:contain!important;overflow:visible!important}
  #paperDoll .pandaEmoji,#paperDoll .panda,#paperDoll .paperDollPanda,#paperDoll .playerEmoji,#paperDoll .characterEmoji{position:relative!important;z-index:1!important;font-size:26px!important;line-height:1!important;transform:scale(.45)!important;transform-origin:center center!important;max-width:36px!important;max-height:36px!important}
}
...
/* Independent weapon overlay used only on mobile. It is outside the paper-doll stacking/clipping context. */
#pandaniaMobileWeaponOverlay{position:fixed!important;display:none!important;pointer-events:none!important;z-index:2147483647!important;width:76px!important;height:76px!important;max-width:none!important;max-height:none!important;object-fit:contain!important;transform:none!important}
@media (max-width:700px),(pointer:coarse){#pandaniaMobileWeaponOverlay.pandaniaVisible{display:block!important}}
</style>
<script>
(function(){'use strict';
...
function getWeaponSlot(){const root=document.querySelector('#paperDoll');if(!root)return null;const sels=['.equippedWeapon','.weaponSlot','#equippedWeapon','#equippedWeaponSlot'];for(const sel of sels){const e=root.querySelector(sel);if(e)return e}return null}
function removeWeaponOverlay(){const o=document.getElementById('pandaniaMobileWeaponOverlay');if(o){o.classList.remove('pandaniaVisible');o.removeAttribute('src')}}
function forcePaperDollWeapon(){
 const root=document.querySelector('#paperDoll');if(!root){removeWeaponOverlay();return}
 root.style.setProperty('overflow','visible','important');
 root.querySelectorAll('*').forEach(el=>el.style.setProperty('overflow','visible','important'));
 const slot=getWeaponSlot();
 if(!slot){removeWeaponOverlay();return}
 slot.style.setProperty('position','relative','important');slot.style.setProperty('z-index','1000','important');slot.style.setProperty('overflow','visible','important');
 const img=slot.querySelector('img');
 if(!img||!img.src){removeWeaponOverlay();return}
 ...
 if(!(matchMedia('(pointer:coarse)').matches||matchMedia('(max-width:700px)').matches)){removeWeaponOverlay();return}
 ...
 /* Keep the whole weapon inside the viewport even in narrow portrait mode. */
 const size=Math.min(76,Math.max(54,window.innerWidth-24));
 let left=r.left+(r.width-size)/2,top=r.top+(r.height-size)/2;
 left=Math.max(6,Math.min(left,window.innerWidth-size-6));
 top=Math.max(6,Math.min(top,window.innerHeight-size-6));
 o.style.width=size+'px';o.style.height=size+'px';o.style.left=Math.round(left)+'px';o.style.top=Math.round(top)+'px';o.classList.add('pandaniaVisible');
}
window.__pandaniaFixEquippedWeaponV14=forcePaperDollWeapon;
...
window.addEventListener('resize',forcePaperDollWeapon,{passive:true});window.addEventListener('orientationchange',()=>setTimeout(forcePaperDollWeapon,120),{passive:true});
setInterval(forcePaperDollWeapon,1000);
setInterval(fixMiniMap,1000);
})();
</script>
